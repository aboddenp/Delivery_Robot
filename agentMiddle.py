...
class Rob_middle_layer(Environment):
    ...
    def do(self, action):
        ...
        while not arrived and remaining != 0:
            # call top layer to see if the commands have changed, top layer should send a flag to notify if cont
            proceed = self.topLayer.proceed(target_pos)
            if not proceed:
                return "moved" # stop moving to this location
